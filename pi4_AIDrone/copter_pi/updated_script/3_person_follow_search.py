#!/usr/bin/env python3
"""
SCRIPT 3 (FINAL): Efficient Person Follow (YOLO11 NCNN) + Lost Wait + Search Scan
===============================================================================
Camera: 640x480
Alt target (safe): 5m
...
Notes (real Pixhawk + Raspberry Pi):
- This script is configured for REAL hardware by default (USE_SITL = False).
- If you are running headless (no monitor), keep DISPLAY = False.

Keys (DISPLAY=True only):
- q : quit
- s : stop (hover)
- l : land
"""
...
# MODE (SITL / REAL)
# =========================
USE_SITL = False  # ✅ REAL hardware default

SITL_CONNECTION = "tcp:127.0.0.1:5762"

# Tip: use stable device name on Raspberry Pi, e.g.:
...
# ✅ Headless-safe default. Set True only if you have GUI/monitor.
DISPLAY = True
...
# MODEL (NCNN)
# =========================
# Resolve model path relative to this script (works regardless of your current working directory)
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
MODEL_PATH = os.path.join(BASE_DIR, "yolo11n_ncnn_model")
PERSON_CLASS = 0
...
# ✅ Safety: during LOST_WAIT / SEARCH, always use full-frame inference
FORCE_FULL_FRAME_WHEN_NOT_TRACKING = True
...
# ✅ Avoid false "lost" from 1 missed frame
MISS_TIMEOUT_S = 0.6
...
class CameraThread:
    def __init__(self, index=0, w=640, h=480):
        # On Raspberry Pi / Linux, V4L2 is usually correct.
        ...
        if not self.cap.isOpened():
            raise RuntimeError(
                "Could not open camera. Try CAM_INDEX=1 or check /dev/video0 permissions."
            )
...
def main():
    ...
    try:
        while True:
            ...
            not_tracking = (state != TRACKING)
            force_full = FORCE_FULL_FRAME_WHEN_NOT_TRACKING and not_tracking

            use_full = force_full or (bbox_smooth is None) or (det_count % FULL_FRAME_EVERY == 0)
            roi_rect = None
            inp = frame
            ...
            # update bbox_smooth + last_seen
            found_this_loop = found is not None
            if found_this_loop:
            ...
            if state == TRACKING:
                # If we never found anyone yet
                if bbox_smooth is None or last_seen_t is None:
                    state = LOST_WAIT
                    lost_start_t = time.time()
                    send_body_vel_yawrate(vehicle, 0, 0, 0, 0)

                else:
                    # If we haven't seen the target recently -> LOST_WAIT
                    if (time.time() - last_seen_t) > MISS_TIMEOUT_S:
                        state = LOST_WAIT
                        lost_start_t = time.time()
                        # clear smoothed commands
                        vx_s = vy_s = yaw_s = 0.0
                        send_body_vel_yawrate(vehicle, 0, 0, 0, 0)

                    else:
                        # If this specific loop didn't detect, hover (safer)
            ...
            if state == LOST_WAIT:
                # hover for N seconds while still running detection
                send_body_vel_yawrate(vehicle, 0, 0, 0, 0)
                ...
                # rotate LEFT (CCW) 180deg relative
                ...
                if (time.time() - search_phase_start_t) >= search_phase_duration:
                    # one scan cycle done -> go back to LOST_WAIT and repeat
                    state = LOST_WAIT
                    lost_start_t = time.time()
                    send_body_vel_yawrate(vehicle, 0, 0, 0, 0)
